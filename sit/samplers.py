...
def euler_sampler(
    ...
    # setup conditioning
    if cfg_scale > 1.0:
        # Use num_classes from the model instead of hardcoded 1000
...
def euler_maruyama_sampler(
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-2], t_steps[1:-1])):
            ...
            # compute drift
            if self_guidance == "True" and cfg_scale > 1. and t_cur <= guidance_high and t_cur >= guidance_low:
                # Self-guidance: use strong model and weak model for guidance
                v_cur_weak = model_weak(model_input.to(dtype=_dtype), time_input.to(dtype=_dtype), **kwargs)[0].to(torch.float64)
                v_cur = model(
                    model_input.to(dtype=_dtype), time_input.to(dtype=_dtype), **kwargs
                    )[0].to(torch.float64)
                # Expand model_input for computing score with expanded v_cur
                model_input_expanded = torch.cat([model_input, model_input], dim=0)
                time_input_expanded = torch.cat([time_input, time_input], dim=0)
                v_cur = torch.cat([v_cur, v_cur_weak], dim=0)
                s_cur = get_score_from_velocity(v_cur, model_input_expanded, time_input_expanded, path_type=path_type)
                d_cur = v_cur - 0.5 * diffusion * s_cur
                # Apply self-guidance: strong - weak
                d_cur_strong, d_cur_weak = d_cur.chunk(2)
                d_cur = d_cur_weak + cfg_scale * (d_cur_strong - d_cur_weak)
            else:
                v_cur = model(
                    model_input.to(dtype=_dtype), time_input.to(dtype=_dtype), **kwargs
                    )[0].to(torch.float64)
