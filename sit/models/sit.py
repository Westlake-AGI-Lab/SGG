...
from models.unet import UNetModel, timestep_embedding
...
class UNetAdapter(nn.Module):
    """
    Adapter to make UNet compatible with SiT interface.
    Wraps UNetModel to match SiT's forward signature.
    """
    def __init__(
        self,
        path_type='edm',
        ...
        **block_kwargs
        ...
        self.unet = UNetModel(
            ...
            model_channels=128,  # default UNet channel size
            out_channels=in_channels,
            num_classes=num_classes if not use_cfg else num_classes + 1,
            attention_resolutions=(2, 4, 8),
            dropout=class_dropout_prob,
    ...
    def forward(self, x, t, y, return_logvar=False, double=False):
        """
        Forward pass adapted to match SiT interface.
        
        Args:
            x: (N, C, H, W) tensor of spatial inputs
            t: (N,) tensor of diffusion timesteps (in [0, 1])
            y: (N,) tensor of class labels
            return_logvar: unused, for compatibility
            double: unused, for compatibility
            
        Returns:
            output: (N, C, H, W) tensor of model predictions
            zs: list of projection outputs (empty for UNet)
        """
        # Convert timesteps from [0, 1] range to UNet's expected range
        # UNet's timestep_embedding can handle [0, 1] range
        timesteps = t.float()
        
        # Prepare extra dict for UNet
        extra = {}
        if self.condition == "uncond":
            labels = torch.tensor([self.num_classes] * y.size(0), device=y.device)
        else:
            labels = y
        
        # UNet expects labels in extra dict
        if labels is not None:
            extra["label"] = labels
        
        # Call UNet forward
        output = self.unet(x, timesteps, extra)
        
        # Return empty zs since UNet doesn't have projectors
        zs = []
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs # fused_attn
        ...
        if weak_type == "Branch" or weak_type == "Segmented":
            self.mid_layer = FinalLayer(hidden_size, patch_size, self.out_channels)
    ...
    def initialize_weights(self):
        ...
        if self.weak_type == "Branch" or self.weak_type == "Segmented":
            nn.init.constant_(self.mid_layer.adaLN_modulation[-1].weight, 0)
            nn.init.constant_(self.mid_layer.adaLN_modulation[-1].bias, 0)
            nn.init.constant_(self.mid_layer.linear.weight, 0)
            nn.init.constant_(self.mid_layer.linear.bias, 0)
    ...
    def forward(self, x, t, y, return_logvar=False, double=False):
        ...
        if self.condition == "uncond":
            y = torch.tensor([self.num_classes] * y.size(0), device=y.device)
        ...
        for i, block in enumerate(self.blocks):
            ...
            # STG style weak to strong
            if double == True and self.weak_type == "LayerSkip" and (i+1) == self.weak_layer:
                x_weak = x.clone()
                x_strong = block(x, c)
                x = torch.cat((x_strong, x_weak), dim=0)
            ...
            # Branching style weak to strong
            if (self.weak_type == "Branch" or self.weak_type == "Segmented") and (i + 1) == self.weak_layer:
                x_mid = self.mid_layer(x, c)
                x_mid = self.unpatchify(x_mid)
...
def UNet_Base(**kwargs):
    """UNet with base configuration"""
    return UNetAdapter(
        in_channels=4,
        hidden_size=128,
