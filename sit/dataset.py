...
class CustomDataset(Dataset):
    def __init__(self, data_dir, num_classes=None, max_images_per_class=None):
        PIL.Image.init()
        ...
        # Filter dataset to only include classes 0 to num_classes-1
        if self.num_classes is not None:
            valid_indices = np.where(self.labels < self.num_classes)[0]
            self.image_fnames = [self.image_fnames[i] for i in valid_indices]
            self.feature_fnames = [self.feature_fnames[i] for i in valid_indices]
            self.labels = self.labels[valid_indices]
        
        # Further filter by max_images_per_class if specified
        if self.max_images_per_class is not None and self.max_images_per_class > 0:
            self._filter_by_max_images_per_class()

    def _filter_by_max_images_per_class(self):
        """
        Filter dataset to include at most max_images_per_class images per class.
        This ensures consistency between training and evaluation datasets.
        """
        ...
        # Select at most max_images_per_class indices per class
        selected_indices = []
        for class_id in sorted(class_indices.keys()):  # Process classes in order for consistency
            indices = class_indices[class_id]
            # Sort indices to ensure consistent selection (deterministic)
            indices.sort()
            # Take at most max_images_per_class indices
            selected_indices.extend(indices[:self.max_images_per_class])
        
        # DO NOT sort selected_indices globally - this disrupts class grouping
        # The DataLoader with shuffle=True will randomize the order anyway
        # Sorting globally can create problematic patterns when max_images_per_class changes
        
        # Update the dataset with selected indices (maintaining class grouping)
        ...
        print(f"Filtered dataset: {len(selected_indices)} images from {len(class_indices)} classes")
        print(f"Max images per class: {self.max_images_per_class}")
        
        # Count images per class after filtering
        filtered_class_counts = {}
        for label in self.labels:
            filtered_class_counts[label] = filtered_class_counts.get(label, 0) + 1
        
        for class_id in sorted(filtered_class_counts.keys()):
            print(f"Class {class_id}: {filtered_class_counts[class_id]} images")
